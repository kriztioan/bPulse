...
SettingsManager::SettingsManager() { _init(0, NULL); }

SettingsManager::SettingsManager(int argc, char *argv[]) { _init(argc, argv); }

int SettingsManager::_init(int argc, char *argv[]) { return 0; }
...
int SettingsManager::WriteSettingsToFile() {
  ...
  for (std::map<std::string, std::string>::iterator option = _Options.begin();
       option != _Options.end(); ++option) {
    ...
    ofstr << "$" << option->first << " = \"" << option->second << "\""
          << std::endl;
  }
